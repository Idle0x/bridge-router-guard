...
This concept is implemented as [`PreAttackMonitorTrap`](../src/concepts/PreAttackMonitorTrap.sol) and tested in [`test/concepts/ConceptTraps.t.sol`](../test/concepts/ConceptTraps.t.sol). The campaign demonstrated it at block 2801775 — `preAttackCampaign` executed 5 failed attempts against `MockPrivilegedBridge`, BridgeRouterGuard returned false (correct — no mismatch), and `failedAttemptCount` rose to 5. See [010 — Architecture and Extensions](./010-architecture-and-extensions.md#trap-3--pre-attack-window-monitor) for the full design.
